...
### 2.4.0 (2026-08-20)
- UIを画像改善案に基づいて全面刷新
  - 統合検索バー（名前・IP・メモ・タグ / Ctrl+K）
  - タグチップフィルター（servers.yaml の tags フィールドで定義）
  - テーブルヘッダー（OS/名前ホスト/タグ/メモ/最終接続/認証/操作）
  - サーバー行にチェックボックス・状態ドット・タグチップ・認証列を追加
  - auth ボタンを廃止し ⋯ メニューに移動、接続ボタン単独化
  - ステータスバーにキーボードショートカット表示
  - servers.yaml に tags フィールドを追加

### 2.3.2 (2026-08-20)
- セットアップウィザードでアップデート時に servers.yaml / config.yaml を上書きしないよう変更
  - EazyConnecter.exe は常に上書き（本体アップデート）
  - servers.yaml / config.yaml は新規インストール時のみ配置
  - 確認画面に「既存のため上書きしない」を表示
